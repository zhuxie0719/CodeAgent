...
class Flask200TestSuite:
    """Flask 2.0.0 专用测试套件"""
    
    def __init__(self):
        self.flask_zip_path = Path("tests/flask-2.0.0.zip")
        self.test_dir = Path("flask_2_0_0_test_project")
        self.flask_source_dir = None
        
    def extract_flask_source(self):
        """从zip包中提取Flask 2.0.0源码"""
        print("📦 提取Flask 2.0.0源码...")
        
        if not self.flask_zip_path.exists():
            print(f"❌ 未找到Flask源码包: {self.flask_zip_path}")
            return False
        
        # 创建临时目录
        self.flask_source_dir = Path(tempfile.mkdtemp(prefix="flask_2_0_0_"))
        
        # 解压Flask源码
        with zipfile.ZipFile(self.flask_zip_path, 'r') as zip_ref:
            zip_ref.extractall(self.flask_source_dir)
        
        # 找到Flask源码目录
        flask_src = self.flask_source_dir / "flask-2.0.0" / "src" / "flask"
        if not flask_src.exists():
            print("❌ Flask源码目录不存在")
            return False
        
        print(f"✅ Flask源码已提取到: {flask_src}")
        return True
    ...
    def create_test_project(self):
        ...
        # 复制Flask源码到测试项目
        flask_dest = self.test_dir / "flask"
        if self.flask_source_dir:
            flask_src = self.flask_source_dir / "flask-2.0.0" / "src" / "flask"
            shutil.copytree(flask_src, flask_dest)
        
        # 创建主测试文件
        self._create_main_test_file()
    ...
    def _create_main_test_file(self):
        """创建主测试文件，包含所有32个Issue"""
...
'''
        ...
        with open(self.test_dir / "test_flask_2_0_0.py", "w", encoding="utf-8") as f:
            f.write(main_content)
    
    def _create_requirements(self):
        """创建requirements.txt"""
        requirements = '''# Flask 2.0.0 依赖
Werkzeug>=2.0
Jinja2>=3.0
itsdangerous>=2.0
click>=7.1.2
...
'''
        ...
        with open(self.test_dir / "run_tests.py", "w", encoding="utf-8") as f:
...
2. 使用检测系统分析：
   ```bash
   python start_api.py
   # 然后上传 flask_2_0_0_test_project 目录
   ```
...
'''
    ...
    def cleanup(self):
        """清理临时文件"""
        if self.flask_source_dir and self.flask_source_dir.exists():
            shutil.rmtree(self.flask_source_dir)
            print("🧹 临时文件已清理")
    ...
    def create_test_project(self):
        ...
        try:
            # 1. 提取Flask源码
            if not self.extract_flask_source():
                return None
            
            # 2. 创建测试项目
            project_path = self.create_test_project()
            
            print(f"✅ 测试项目已创建: {project_path}")
            return project_path
