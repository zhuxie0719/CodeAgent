...
from fixcodeagent.run.hello_world import main
...
def test_run_hello_world_end_to_end(local_test_data):
    ...
    with (
        patch("fixcodeagent.run.hello_world.LitellmModel") as mock_model_class,
        patch("os.environ", {"FIXCODE_MODEL_NAME": "tardis"}),
    ):
