...
class _TextualAgent(DefaultAgent):
    ...
    def execute_action(self, action: dict) -> dict:
        ...
        # Automatically confirm completion commands
        action_str = action["action"].strip()
        is_completion_command = "COMPLETE_TASK_AND_SUBMIT_FINAL_OUTPUT" in action_str or "FIX_CODE_AGENT_FINAL_OUTPUT" in action_str
        if (
            self.config.mode == "confirm"
            and action_str
            and not is_completion_command
            and not any(re.match(r, action_str) for r in self.config.whitelist_actions)
        ):
    ...
    def has_finished(self, output: dict[str, str]):
        try:
            ...
            # Check if this is a completion command - if so, auto-confirm exit
            # Use the same logic as DefaultAgent: check if first non-empty, non-comment line is the marker
            raw_output = output.get("output", "")
            is_completion_command = False
            if raw_output:
                ...
                for line in lines:
                    ...
                    if stripped in {"FIX_CODE_AGENT_FINAL_OUTPUT", "COMPLETE_TASK_AND_SUBMIT_FINAL_OUTPUT"}:
                        is_completion_command = True
                        break
            
            if self.config.confirm_exit and not is_completion_command:
