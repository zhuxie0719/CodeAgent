...
def ping():
    ...
    if not host.replace('.', '').isdigit():
        return jsonify({'error': '无效的主机地址'}), 400
...
def process_data():
    ...
    except (pickle.UnpicklingError, EOFError, AttributeError, ImportError, IndexError, TypeError, ValueError, base64.binascii.Error):
        return jsonify({'error': '处理失败: 数据格式错误'}), 400
